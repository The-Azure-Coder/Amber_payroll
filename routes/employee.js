...
router.get("/", (req, res) => {
  let employees = `SELECT et.id, lt.id as login_id, et.first_nm,et.last_nm, et.nis, et.trn, lt.email, lt.password, rt.id AS role_id, dt.id AS depart_id, dt.department_nm, rt.roles,et.expected_hrs FROM payroll.employees et JOIN payroll.logins lt ON lt.emp_id = et.id JOIN payroll.departments dt ON lt.department_id = dt.id JOIN payroll.roles rt ON lt.role_id = rt.id`;
...
router.get('/edit/:id',(req,res)=>{
    ...
    let editSql = `SELECT et.id, lt.id as login_id, et.first_nm,et.last_nm, et.nis, et.trn, lt.email, lt.password, rt.id AS role_id, dt.id AS depart_id, dt.department_nm, rt.roles,et.expected_hrs FROM payroll.employees et JOIN payroll.logins lt ON lt.emp_id = et.id JOIN payroll.departments dt ON lt.department_id = dt.id JOIN payroll.roles rt ON lt.role_id = rt.id WHERE et.id = ${id}`
    let roles = `SELECT * FROM roles`
    let departments = `SELECT * FROM departments`
...
router.post("/update", (req, res) => {
    let updateEmp = `update employees SET department_id='${req.body.department_id}', first_nm='${req.body.first_nm}', last_nm='${req.body.last_nm}', nis='${req.body.nis}', trn='${req.body.trn}', expected_hrs='${req.body.expected_hrs}' where id =${req.body.id}`;
